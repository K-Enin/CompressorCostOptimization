...
# add1: compressor can at least switch e.g. 2 times,
# add2: compressor can switch after certain time period
add = 'add1'
if additional_constraints: # only for 'add2'
    M1, M2 = 1/12, 1/12 #(5 time steps)
...
def gasnetwork_nlp(P_time0, Q_time0, P_initialnode, Q_lastnode, eps, Edgesfile):
    ...
    input: P_initialnode, Q_lastnode, eps
    ...
    for j in list_outgoing_edges:
        if j != number_of_edges_without_slack_edge: # not last edge    
...
if __name__ == '__main__':
     ...
     if folder == 'Example_Advanced/':
         ...
         eps = mat_file["eps"]
         eps[0,0] = round(eps[0,0],4) # to match with initial data
