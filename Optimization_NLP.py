...
def gasnetwork_nlp(P_time0, Q_time0, eps, Edgesfile):
    ...
    if (np.shape(P_time0) != np.shape(Q_time0)):
        raise Exception("Mitsmatch of dimension of Q_time0, P_time0.")
    ...
    for edge in range(number_of_edges_without_slack_edge):
        ...
        if edge in starting_edges: 
            ...
            ubw += [*P_time0[edge]] + [ubP]*(n)*(m-1)
            # Go through Q
            w0 += [*Q_time0[edge]] + [InitialGuess_P_middleEdge]*(n)*(m-1)
            lbw += [*Q_time0[edge]] + [lbQ]*(n)*(m-1)
            ubw += [*Q_time0[edge]] + [ubQ]*(n)*(m-1)
        else: 
            # Go through P
    ...
    for node in nodes_list: 
        ...
        for edge_in in ingoing_edges:
            ...
            # Only for edges which are sink edge (thus slack edge)
    ...
    for j in list_outgoing_edges:
        if j != number_of_edges_without_slack_edge:
...
def plot_solution(u, P, Q, parameters, final_directory):
    """
    ...
    axes.set(xlabel = r'time step $t$', ylabel = 'pressure (bar) at node 0')
    ...
    axes.set(xlabel = r'time step $t$', ylabel = 'mass flow (kg/s) at node 6')
...
if __name__ == '__main__':
     ...
     final_directory = os.path.join(current_directory, folder + '/optimization_nlp/')
     ...
     options = {'ipopt': {'tol': 1e-8, 'max_iter': 2000, 'linear_solver': 'ma27'}}
     ...
     u, P, Q = extract_solution(sol, parameters)
     
     t_stop = process_time()
     
     plot_solution(u, P, Q, parameters, final_directory)
