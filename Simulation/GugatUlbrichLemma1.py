...
the paper.
Simulation needs quite some time!
"""
import numpy as np
...
def EulerGleichung_LxF():
    ...
    # dt = (CFL*dx)/(np.amax(abs(Q_Euler/P_Euler) + a_square*np.ones((1,n+1)))) in the while loop below
    # but since dt is so small it does fulfill the CFL
    dt = 0.0001
...
def EulerGleichung_SimpleUp():
    ...
    while T < 2:
        ...
        # Compatibility conditions
        P_Euler_new[0,0] = 4.75*T*T+1
        # last step in n is not needed for Simple Upwind, becasue it calculates this 
        # P_Euler_new[n,0] = P_Euler[n,0]
